...
if __name__ == "__main__":
    ...
    for original_modelName, original_modelClass in zip(modelNames, modelClasses):
        for attack_modelName, attack_modelClass in zip(modelNames, modelClasses):
            if original_modelName != attack_modelName and original_modelName == 'ResNet50':
                print('Model under attack ... ', attack_modelName)
                for scenario in scenarios:
                    main_folder = f'runs_final/{scenario}'
                    classif = attack_modelClass
                    checkpoint_file = f'../pytorch-cifar-master/checkpoint/{attack_modelName}/ckpt.pth'
                    ...
                    for folder_name in os.listdir(main_folder):
                        if folder_name.startswith(original_modelName):
                            ...
                            all_indices = pd.read_csv(f'correctly_classified_images/{attack_modelName}_correctly_classified_images.csv')['image id']
                            used_indices = pd.read_csv(f'selected_images_EvoSet/{original_modelName}_cifar_attack_subset_2020_5x200.csv')['index']
                            used_indices =  [index for index in all_indices.values if index in used_indices.values]  
                            unused_indices = [index for index in all_indices.values if index not in used_indices]        
                            ...
                            csv_file_path = f"{run_folder}/{attack_modelName}_test_info.csv"
                            ...
                            # Filter adversarial images (those in wrong_classification)
                            ...
                            # Save the grid image
                            pyplot.savefig(f'{run_folder}/{attack_modelName}_adversarial_examples.png', bbox_inches='tight')
                            pyplot.close()
