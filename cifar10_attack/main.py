...
def compute_linf_distance_gpu(original_image, perturbed_image):
    # Compute the L∞ distance between two images
    linf_distances = []
    for img1, img2 in zip(original_image, perturbed_image):
        linf_distance = torch.max(torch.abs(img1/255 - img2/255))
        linf_distances.append(linf_distance.item())  # Convert to Python scalar
    return linf_distances
   ...
   # print("Generation: ", generation)
    ...
    with tf.device('/gpu:0'):
        ...
        for tensor in tensors:
            ...
            for p in range(5):
                patch = patches[p]
                # patch_cuda = torch.tensor(patch, dtype=torch.float32).to('cuda')
...
if __name__ == "__main__":
    ...
    # new params
    total_subset_size = 1000 # 2000
    mini_subset_size = 200 # 100
    ...
    for sce in scenarios:
        ...
        for m in range(len(modelNames)):
            ...
            selected_images_file = pd.read_csv(f'selected_images_EvoSet/{modelName}_cifar_attack_subset_2020_{qnt_subsets}x{mini_subset_size}.csv')
            random_indices = selected_images_file['index']
